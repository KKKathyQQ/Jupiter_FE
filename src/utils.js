...
const SERVER_ORIGIN = ''; //or deployed server address
                          // but we deploy front and back together, leave it default
                          // if deploy frontend alone, need to fill in the server address

const loginUrl = `${SERVER_ORIGIN}/login`;
 
export const login = (credential) => {
  return fetch(loginUrl, { // send request
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    credentials: 'include', // include cookie
    body: JSON.stringify(credential) // 扁平化，非结构化，就是string
  }).then((response) => { // the return from the function is an object
